...
/* END - RESTORE POINT - by MR. MILK August 31, 2005 02:22:18 PM */
	...
	function rebuild_languages($languages = array(), $modules="")
	{
            foreach($languages as $language=>$value){
				...
				// BEGIN syslanguages
                if($modules =="" &&
                    isset($GLOBALS['sugar_config']['syslanguages']['spicecrmsource_overwrite_with_db']) &&
                    $GLOBALS['sugar_config']['syslanguages']['spicecrmsource_overwrite_with_db'] === true){
                    //$allModules = array_merge($GLOBALS['moduleList'], $GLOBALS['modInvisList']);
                    foreach ($GLOBALS['moduleList'] as $idx => $module){
                        LanguageManager::createLanguageFileUIOverride($module, $language);
                    }
                }
                //END
