...
class aSubPanel
{
    ...
    function load_sub_subpanels ()
    {
        ...
        if (empty ( $this->sub_subpanels ))
        {
            ...
            foreach ( $panels as $panel => $properties )
            {
                ...
                //BEGIN 20reasons CR1000163: for search function in subpanel in collection: assign search_config values to subpanel
                if($this->isCollection() && is_array($this->_instance_properties['collection_list'][$panel]['search_config'])){
                    $properties['search_config'] = $this->_instance_properties['collection_list'][$panel]['search_config'];
                }elseif(is_array($this->_instance_properties['search_config'])){
                    $properties['search_config'] = $this->_instance_properties['search_config'];
                }
                //END
            ...
            foreach ( $this->sub_subpanels as $key => $subpanel )
            {
                ...
                //begin 20reasons CR1000163: introduced in spicecrm 201903001 for search function in subpanel in collection: assign search_config values to subpanel panel_definition
                if(is_array($subpanel->_instance_properties['search_config'])) {
                    $subpanel->panel_definition['search_config'] = $subpanel->_instance_properties['search_config'];
                }
                //end
                // begin CR1000187 modified maretval 2019-03-31 introduced in spicecrm 201903001: property value dependency for subpanel display
                if($this->isCollection() && is_array($this->_instance_properties['collection_list'][$panel]['trdisplay_for'])){
                    $properties['trdisplay_for'] = $this->_instance_properties['collection_list'][$panel]['trdisplay_for'];
                }elseif(is_array($this->_instance_properties['trdisplay_for'])){
                    $properties['trdisplay_for'] = $this->_instance_properties['trdisplay_for'];
                }
                // end
    ...
    function get_where ()
    {
        ...
        if($this->panel_definition['search_config']['searchable'] === true){
            $custom_where = \SpiceCRM\includes\SubPanel\SubPanelSearch::get_searchfield_where($this->panel_definition['search_config'], $_REQUEST['search_subpanel']);
        }
        //because of changes for collection list, we need to check content here too
		elseif($this->_instance_properties['search_config']['searchable'] === true){
            $custom_where = \SpiceCRM\includes\SubPanel\SubPanelSearch::get_searchfield_where($this->_instance_properties['search_config'], $_REQUEST['search_subpanel']);
        }
        if(!empty($custom_where)){
            $this->panel_definition['where'] .= (!empty($this->panel_definition['where']) ? " AND " : '').$custom_where;
        }
        //end
...
class SubPanelDefinitions
{
    ...
    function load_subpanel ( $name , $reload = false , $original_only = false )
    {
        ...
        // begin CR1000141 ACL hide subpanels the user is not allowed to see (they correspond to module)
        if (!$GLOBALS['ACLController']->checkAccess($this->layout_defs['subpanel_setup'][strtolower($name)][ 'module' ], 'view')) {
            return false;
        }
        // end

        // begin CR1000187 modified maretval 2019-03-31 introduced in spicecrm 201903001: property value dependency for subpanel display
        if(isset($this->layout_defs[ 'subpanel_setup' ][ strtolower ( $name ) ] [ 'trdisplay_for' ]) && !empty($this->layout_defs[ 'subpanel_setup' ][ strtolower ( $name ) ] [ 'trdisplay_for' ])){
            $displays = array();
            foreach($this->layout_defs[ 'subpanel_setup' ][ strtolower ( $name ) ] [ 'trdisplay_for' ] as $beingFlag){
                if($this->_focus->$beingFlag) $displays[] = true;
                else $displays[] = false;
            }
            if(!in_array(1, $displays))  return false;
        }
        // end
