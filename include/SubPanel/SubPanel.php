...
class SubPanel
{
    ...
    function ProcessSubPanelListView($xTemplatePath, &$mod_strings)
    {
        ...
        //BEGIN 20reasons CR1000163: introduced in spicecrm 201903001 search function in subpanel
        if(isset($this->subpanel_defs->_instance_properties['search_config']['limit'])){
            $ListView->records_per_page = \SpiceCRM\includes\SubPanel\SubpanelSearch::get_limit($this->subpanel_defs->_instance_properties['search_config'], $this->subpanel_defs->name, $this->subpanel_defs->parent_bean->module_name);
            $ListView->setRecordsPerPage($ListView->records_per_page);
        }elseif(isset($this->subpanel_defs->panel_definition['search_config']['limit'])){
            $ListView->records_per_page = \SpiceCRM\includes\SubPanel\SubpanelSearch::get_limit($this->subpanel_defs->panel_definition['search_config'], $this->subpanel_defs->name, $this->subpanel_defs->parent_bean->module_name);
            $ListView->setRecordsPerPage($ListView->records_per_page);
        }
        //END
