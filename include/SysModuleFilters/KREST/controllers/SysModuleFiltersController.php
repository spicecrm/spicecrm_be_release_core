...
class SysModuleFiltersController
{
    ...
    function saveFilter($req, $res, $args)
    {
        ...
        if ($filter) {
            ...
            $db->query("UPDATE $table SET name='{$filterdata['name']}', filterdefs='$filterdefs', version='{$filterdata['version']}', package='{$filterdata['package']}' WHERE id = '{$args['filter']}'");

            $this->setCR('I', $args['filter'], "{$args['module']}/{$filterdata['name']}");
        } else {
            $filterdefs = json_encode($filterdata['filterdefs']);
            $db->query("INSERT INTO $table (id, created_by_id, module, name, filterdefs, version, package) VALUES('{$args['filter']}', '$current_user->id', '{$args['module']}', '{$filterdata['name']}', '$filterdefs', '{$filterdata['version']}', '{$filterdata['package']}')");
