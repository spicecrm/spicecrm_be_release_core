...
function make_sugar_config(&$sugar_config)
{
    ...
    $sugar_config = array(
        ...
        'default_theme' => empty($default_theme) ? 'SpiceTheme' : $default_theme,
...
function get_sugar_config_defaults() {
    ...
    $sugar_config_defaults = array(
        ...
        'default_theme' => return_session_value_or_default('site_default_theme', 'SpiceTheme'),
...
function securexsskey($value, $die = true) {
    global $sugar_config;
    $matches = array();
    //begin maretval 2016-05-23
    //preg_match('/[\'"<>]/', $value, $matches);//not working under php5.6 because of misinterpretation of []
...
function create_export_query_relate_link_patch($module, $searchFields, $where) {
    ...
    foreach ($seed->field_defs as $name => $field) {
        ...
        if ($field['type'] == 'relate' && isset($field['link']) && !empty($searchFields[$name]['value'])) {
            ...
            $field_link = $field['link']; //PHP7 COMPAT
            $join = $seed->$field_link->getJoin($params, true); //PHP7 COMPAT
...
function floatAsString($float, $removeSeparator = false) {
    $s = strval(floatval($float));
    $e = strrchr($s, 'E');
    if ($e === false) {
        if ($removeSeparator) {
            $s = str_replace(".", "", $s);
    ...
    $s = number_format($s, -intval(substr($e, 1)));
    if ($removeSeparator) {
        $s = str_replace(".", "", $s);
    }
    return $s;
