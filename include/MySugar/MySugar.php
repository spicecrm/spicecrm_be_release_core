...
class MySugar
{
    ...
    function checkDashletDisplay () {
        ...
        elseif ($GLOBALS['ACLController']->moduleSupportsACL($this->type) ) {
            $bean = SugarModule::get($this->type)->loadBean();
            if ( !$GLOBALS['ACLController']->checkAccess($this->type,'list',true,$bean->acltype)) {
                $displayDashlet = false;
            }
            $displayDashlet = true;
        }
...
EOJS;
    ...
    function configureDashlet(){
        ...
        if(!empty($_REQUEST['id'])) {
            ...
            $dashletLocation = get_custom_file_if_exists($dashletDefs[$id]['fileLocation']);

            require_once($dashletLocation);
