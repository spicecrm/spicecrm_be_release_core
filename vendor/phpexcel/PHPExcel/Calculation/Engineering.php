...
class PHPExcel_Calculation_Engineering {
	...
	public static function _parseComplex($complexNumber) {
		...
		if (strlen($workString) > 0) {
			$leadingSign = ((substr($workString, 0, 1) == '+') || (substr($workString, 0, 1) == '-')) ? 1 : 0;
		}
		$power = '';
	...
	private static function _cleanComplex($complexNumber) {
		if (substr($complexNumber, 0, 1) == '+') $complexNumber = substr($complexNumber,1);
		if (substr($complexNumber, 0, 1) == '0') $complexNumber = substr($complexNumber,1);
		if (substr($complexNumber, 0, 1) == '.') $complexNumber = '0'.$complexNumber;
		if (substr($complexNumber, 0, 1) == '+') $complexNumber = substr($complexNumber,1);
