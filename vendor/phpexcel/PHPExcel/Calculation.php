...
class PHPExcel_Calculation {
	...
	public static function _unwrapResult($value) {
		if (is_string($value)) {
			...
			if ((isset($valueChars[0])) && ($valueChars[0] == '"') && (substr($value,-1) == '"')) {
	...
	public function parseFormula($formula) {
        ...
        $formulaChars = str_split($formula, 1);
		if ((!isset($formulaChars[0])) || ($formulaChars[0] != '=')) return array();
		$formula = ltrim(substr($formula,1));
        $formulaChars = str_split($formula, 1);
		if (!isset($formulaChars[0])) return array();
...
//echo 'Retrieve from cache',PHP_EOL;
	...
	public function _calculateFormulaValue($formula, $cellID=null, PHPExcel_Cell $pCell = null) {
		...
		$formula = ltrim(substr($formula,1));
        $formulaChars = str_split($formula, 1);
        if (!isset($formulaChars[0])) return self::_wrapResult($formula);
		...
		if ((substr($wsTitle, 0, 1) !== "\x00") && ($this->_cyclicReferenceStack->onStack($wsTitle.'!'.$cellID))) {
...
//echo 'Assessing Expression '.substr($formula, $index),PHP_EOL;
			$opCharacter = $formulaChars[$index];	//	Get the first character of the value at the current index position
//echo 'Initial character of expression block is '.$opCharacter,PHP_EOL;
			if ((isset(self::$_comparisonOperators[$opCharacter])) && (strlen($formula) > $index) && (isset(self::$_comparisonOperators[$formulaChars[$index+1]]))) {
				$opCharacter .= $formulaChars[++$index];
...
//						echo 'Element '.$localeConstant.' is an Excel Constant<br />';
			...
			//	Ignore white space
            $formulaChars = str_split($formula, 1);
			while (($formulaChars[$index] == "\n") || ($formulaChars[$index] == "\r")) {
				++$index;
			}
			if ($formulaChars[$index] == ' ') {
				while ($formulaChars[$index] == ' ') {
					++$index;
				}
...
//		}
	...
	public function _raiseFormulaError($errorMessage) {
