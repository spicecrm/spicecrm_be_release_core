...
namespace SpiceCRM\KREST\utils;

class KRESTRateLimiter {
    ...
    static function loadData( $userId ) {
        if ( self::$fp = fopen( session_save_path().'/rali_'.$GLOBALS['sugar_config']['unique_key'].'_'.$userId, 'c+' )) {#} and flock( self::$fp, LOCK_EX )) {
            ...
            throw new \SpiceCRM\KREST\ToManyRequestsException();
        }
    }
    ...
    static function check( $httpMethod ) {
        ...
        # The blocking:
        if ( $doBlock ) throw ( new \SpiceCRM\KREST\ToManyRequestsException())->setRetryAfter( $retryAfter );
