...
// for KREST as standalone. Skip this route!
// LogViewer is a spicecrm capability and will not work on other sugar based crms
if(file_exists('include/SugarLogger/LogViewer.php')) {
    $app->group('/crmlog', function () use ($app) {
        ...
        $this->get('', function ($req, $res, $args) {
            ...
            $lines = $viewer->getAllLines($req->getQueryParams());
            return $res->withJson([
        ...
        $this->get('/file/{filenr:\d+}', function ($req, $res, $args) {
            $viewer = new LogViewer();
            $lines = [];
            $viewer->getLinesOfFile($args['filenr'], $lines, $req->getQueryParams());
            return $res->withJson([
                'currentLogLevel' => @$GLOBALS['sugar_config']['logger']['level'],
                'count' => count($lines),
                'lines' => $lines
        ...
        $app->get('/day/{date:\d{8}}', function ($req, $res, $args) {
            $viewer = new LogViewer();
            $lines = $viewer->getLinesOfPeriod('day', [$args['date']], $req->getQueryParams());
            return $res->withJson([
                'currentLogLevel' => @$GLOBALS['sugar_config']['logger']['level'],
                'count' => count($lines),
                'lines' => $lines
        ...
        $app->get('/month/{yearmonth:\d{6}}', function ($req, $res, $args) {
            $viewer = new LogViewer();
            $lines = $viewer->getLinesOfPeriod('month', [$args['yearmonth']], $req->getQueryParams());
            return $res->withJson([
                'currentLogLevel' => @$GLOBALS['sugar_config']['logger']['level'],
                'count' => count($lines),
                'lines' => $lines
    ...
    $app->get( '/year/{year:\d{4}}', function( $req, $res, $args ) {
        $viewer = new LogViewer();
        $lines =  $viewer->getLinesOfPeriod( 'year', [ $args['year'] ], $req->getQueryParams() );
        return $res->withJson([
            'currentLogLevel' => @$GLOBALS['sugar_config']['logger']['level'],
            'count' => count( $lines ),
            'lines' => $lines
    ...
    $app->get( '/day/{date:\d{8}}/hour/{hour:\d{2}}', function( $req, $res, $args ) {
        $viewer = new LogViewer();
        $lines =  $viewer->getLinesOfPeriod( 'hour', [ $args['date'], $args['hour'] ], $req->getQueryParams() );
        return $res->withJson([
            'currentLogLevel' => @$GLOBALS['sugar_config']['logger']['level'],
            'count' => count( $lines ),
            'lines' => $lines
        ...
        $app->get('/fullLine/{filenr:\d+}/{linenr:\d+}', function ($req, $res, $args) {
            $viewer = new LogViewer();
            $line = $viewer->getFullLine($args['filenr'], $args['linenr'], $req->getQueryParams());
            return $res->withJson([
                'currentLogLevel' => @$GLOBALS['sugar_config']['logger']['level'],
                'line' => $line
            ]);
