...
function handleDbCreateDatabase() {
    ...
    global $setup_db_admin_user_name;
    global $setup_db_admin_password;
    global $setup_db_lc_collate; //postgres
    global $setup_db_lc_type; //postgres
    global $sugar_config;

    echo "{$mod_strings['LBL_PERFORM_CREATE_DB_1']} {$setup_db_database_name} {$mod_strings['LBL_PERFORM_CREATE_DB_2']} {$setup_db_host_name}...";
    $db = getDbConnection();
    //postgres additional params
    switch($db->dbType){
        case 'pgsql':
            if(empty($setup_db_lc_collate)) $setup_db_lc_collate = "en_US.UTF-8";
            if(empty($setup_db_lc_type)) $setup_db_lc_collate = "en_US.UTF-8";
            $db->createDatabase($setup_db_database_name, $setup_db_lc_collate, $setup_db_lc_type);
            break;
        default:
...
function handleSugarConfig() {
    ...
    $sugar_config['logger']	=
        array ('level'=>$setup_site_log_level,
            'default' => 'SpiceLogger', //introduced SpiceCRM 20180900
            'file' => array(
                'ext' => '.log',
                'name' => 'sugarcrm',
                'dateFormat' => '%c',
