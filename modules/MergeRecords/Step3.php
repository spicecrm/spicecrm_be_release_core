...
foreach ($temp_field_array as $field_array) {
    ...
    if (show_field($field_array)
        ...
        //BEGIN PHP7 COMPAT
        //ORIGINAL: $select_row_curr_field_value = $focus->merge_bean->$field_array['name'];
        $field_array_name = $field_array['name'];
        $select_row_curr_field_value = $focus->merge_bean->$field_array_name;
        //END
        foreach ($merge_ids_array as $id) {
        //BEGIN PHP7 COMPAT
        //ORIGNINAL:    if (($mergeBeanArray[$id]-> $field_array['name']=='' and $select_row_curr_field_value =='') or $mergeBeanArray[$id]-> $field_array['name'] == $select_row_curr_field_value ) {
            if (($mergeBeanArray[$id]->$field_array_name=='' and $select_row_curr_field_value =='') or $mergeBeanArray[$id]->$field_array_name == $select_row_curr_field_value ) {
        //END
                $section_name='merge_row_similar';
            } else  {
        ...
        switch ($field_check) {
            ...
            case ('link') :
                ...
                if (empty($select_row_curr_field_value)) {
                    //BEGIN PHP7 COMPAT
                    //ORIGINAL: $related_name=get_related_name($field_array,$focus->merge_bean->$field_array['id_name']);
                    $field_array_id_name = $field_array['id_name'];
                    $related_name=get_related_name($field_array,$focus->merge_bean->$field_array_id_name);
                    //END
                ...
                //BEGIN PHP7 COMPAT
                //ORIGINAL $xtpl->assign("POPUP_ID_VALUE", $focus->merge_bean->$field_array_id_name);
                $field_array_id_name = $field_array['id_name'];
                $xtpl->assign("POPUP_ID_VALUE", $focus->merge_bean->$field_array_id_name);
                //END
        ...
        //BEGIN PHP7 COMPAT: replace all ->$field_array['name'] below
        $field_array_name = $field_array['name'];
        $field_array_id_name = $field_array['id_name'];
        //END
        ...
        foreach ($merge_ids_array as $id) {
            ...
            switch ($field_check) {
                case ('bool') :
                    if (($mergeBeanArray[$id]->$field_array_name == '1' || $mergeBeanArray[$id]->$field_array_name == 'yes' || $mergeBeanArray[$id]->$field_array_name == 'on') && !empty($mergeBeanArray[$id]->$field_array_name)) {
                    ...
                    break;
                case ('enum') :
                    if(!empty($field_array['function']) && !empty($field_array['options']) && !empty($mergeBeanArray[$id]->$field_array_name)){
                        //if fieldoptions were set from a function, get the value directly
                        display_field_value($field_array['options'][$mergeBeanArray[$id]->$field_array_name]);
                    }else if ($mergeBeanArray[$id]->$field_array_name != '' and isset($field_array['options']) and isset($app_list_strings[$field_array['options']][$mergeBeanArray[$id]->$field_array_name])) {
                        display_field_value( $app_list_strings[$field_array['options']][$mergeBeanArray[$id]->$field_array_name]);
                    } else {
                        display_field_value($mergeBeanArray[$id]->$field_array_name);
                    }
                ...
                case ('multienum') :
                     if(!empty($field_array['function']) && !empty($field_array['options']) && !empty($mergeBeanArray[$id]->$field_array_name)){
                        //if fieldoptions were set from a function, get the value directly
                        $displayME = str_replace("^","",$mergeBeanArray[$id]->$field_array_name);
                        ...
                        display_field_value(str_replace("^","",$app_list_strings[$field_array['options']][$mergeBeanArray[$id]->$field_array_name]));
                    } else {
                        display_field_value(str_replace("^","",$mergeBeanArray[$id]->$field_array_name));
                    }
                    $field_name="main.".$section_name.".merge_cell_field_value";
                    break;
                case ('relate') :
                case ('link') :
                    $related_name=false;
                    if (empty($mergeBeanArray[$id]->$field_array_name) && !empty($mergeBeanArray[$id]->$field_array_id_name)) {
                       $related_name=get_related_name($field_array,$mergeBeanArray[$id]->$field_array_id_name);
                       if ($related_name !== false) {
                            $mergeBeanArray[$id]->$field_array_name=$related_name;
                       }
                    }
               	    display_field_value($mergeBeanArray[$id]->$field_array_name);
                    $field_name="main.".$section_name.".merge_cell_field_value";
            ...
            if ($field_check == 'relate' or $field_check == 'link') {
                $temp_array = Array ();
                $json_data['popup_fields'] = Array ($field_array['name'] => $mergeBeanArray[$id]->$field_array_name, $field_array['id_name'] => $mergeBeanArray[$id]->$field_array_id_name,);
                ...
                $json_data['field_value'] = unencodeMultienum($mergeBeanArray[$id]->$field_array_name);
            } else {
                $json_data['field_value'] = $mergeBeanArray[$id]->$field_array_name;
            }
