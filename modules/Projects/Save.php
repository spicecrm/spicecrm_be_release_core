...
if(isset($_REQUEST['save_type']) || isset($_REQUEST['duplicateSave']) && $_REQUEST['duplicateSave'] === "true") {
    $query = "SELECT id FROM projecttasks WHERE project_id = '" . $base_project_id . "' AND deleted = 0";
...
if ($sugarbean->is_template){
    header("Location: index.php?action=ProjectTemplatesDetailView&module=Projects&record=$return_id&return_module=Projects&return_action=ProjectTemplatesEditView");
}
else{
    handleRedirect($return_id,'Projects');
}
?>