...
//function all string that match the pattern {.} , also catches the list of found strings.
	...
	function parse_template_bean($string, $bean_name, &$focus) {
		...
		if($bean_name == 'Contacts') {
			...
			if(!empty($acct->id)) {
				foreach($acct->field_defs as $field_def) {
                    ...
                    $field_def_name = $field_def['name']; //PHP7 COMPAT
					if($field_def['type'] == 'enum') {
                        $translated = translate($field_def['options'], 'Accounts', $acct->$field_def_name); //PHP7 COMPAT
                        ...
                        $translated = self::_convertToType($field_def['type'], $acct->$field_def_name); //PHP7 COMPAT
                        $repl_arr = EmailTemplate::add_replacement($repl_arr, $field_def, array(
                            'account_'         . $field_def['name'] => $translated,
                            'contact_account_' . $field_def['name'] => $translated,
                        ));
			...
			foreach($contact->field_defs as $field_def) {
                ...
                $field_def_name = $field_def['name']; //PHP7 COMPAT
				if($field_def['type'] == 'enum') {
                    $translated = translate($field_def['options'], 'Accounts', $contact->$field_def_name); //PHP7 COMPAT
                    ...
                    if (isset($contact->$field_def_name)) { //PHP7 COMPAT
                        // bug 47647 - allow for fields to translate before adding to template
                        $translated = self::_convertToType($field_def['type'], $contact->$field_def_name); //PHP7 COMPAT
                        $repl_arr = EmailTemplate::add_replacement($repl_arr, $field_def, array(
                            'contact_'         . $field_def['name'] => $translated,
                            'contact_account_' . $field_def['name'] => $translated,
                        ));
		...
		foreach($focus->field_defs as $field_def) {
            $field_def_name = $field_def['name']; //PHP7 COMPAT
            if (isset($focus->$field_def_name)) { //PHP7 COMPAT
				if(($field_def['type'] == 'relate' && empty($field_def['custom_type'])) || $field_def['type'] == 'assigned_user_name') {
             		continue;
				}

				if($field_def['type'] == 'enum' && isset($field_def['options'])) {
                    $translated = translate($field_def['options'], $bean_name, $focus->$field_def_name); //PHP7 COMPAT
                    ...
                    $repl_arr = EmailTemplate::add_replacement($repl_arr, $field_def, array(
                        strtolower($beanList[$bean_name]) . "_" . $field_def['name'] => self::_convertToType($field_def['type'], $focus->$field_def_name), //PHP7 COMPAT
                    ));
				}
			} else {
    ...
    function _parseUserValues($repl_arr, &$user)
    {
        foreach ($user->field_defs as $field_def) {
            ...
            $field_def_name = $field_def['name']; //PHP7 COMPAT
            if ($field_def['type'] == 'enum') {
                $translated = translate($field_def['options'], 'Users', $user->$field_def_name); //PHP7 COMPAT
                ...
                if (isset($user->$field_def_name)) { //PHP7 COMPAT
                    // bug 47647 - allow for fields to translate before adding to template
                    $repl_arr["contact_user_" . $field_def['name']] = self::_convertToType($field_def['type'], $user->$field_def_name); //PHP7 COMPAT
                } else {
                    $repl_arr["contact_user_" . $field_def['name']] = "";
                }
			}
