...
class Note extends SugarBean {
    ...
    {
        global $current_user;
        $query =  "SELECT notes.id, date_entered sortdate, 'Notes' module FROM notes where ((parent_type = '$parentModule' and parent_id = '$parentId') OR notes.contact_id='$parentId') and notes.deleted = 0";

        switch ($own) {
            case 'assigned':
                $query .= " AND notes.assigned_user_id='$current_user->id'";
                break;
            case 'created':
                $query .= " AND notes.created_by='$current_user->id'";
                break;
        }
        return $query;
    }
