...
$current_module_strings = return_module_language($current_language, 'ProjectTasks');
...
$where = "projecttasks.assigned_user_id='{$current_user->id}'"
	. " AND (projecttasks.status IS NULL OR (projecttasks.status!='Completed' AND projecttasks.status!='Deferred'))"
	. " AND (projecttasks.date_start IS NULL OR projecttasks.date_start <= '$today')";
$ListView->initNewXTemplate('modules/ProjectTasks/MyProjectTasks.html',
