...
class ProjectTask extends SugarBean {
	...
	var $table_name = 'projecttasks';
	var $object_name = 'ProjectTask';
	var $module_dir = 'ProjectTasks';
   ...
   function fill_in_additional_list_fields()
   {
	...
	function _get_depends_on_name($depends_on_id)
	{
		...
		if($row != null)
		{
			...
			$this->depends_on_name_mod = 'ProjectTasks';
    ...
    function _get_project_name($project_id)
    {
        ...
        $query  = "SELECT name, assigned_user_id FROM projects WHERE id='{$project_id}'";
        $result = $this->db->query($query,true," Error filling in additional detail fields: ");
        $row = $this->db->fetchByAssoc($result);
        if($row != null)
        {
	...
	function build_generic_where_clause ($the_query_string)
	{
		...
		array_push($where_clauses, "projecttasks.name like '$the_query_string%'");
    ...
    function create_export_query(&$order_by, &$where, $relate_link_join='')
    {
        ...
        $query .= " FROM projecttasks LEFT JOIN project ON projecttasks.project_id=projects.id AND projects.deleted=0 ";

        $query .=  $custom_join['join'];
        $query .= " LEFT JOIN users
                   	ON projecttasks.assigned_user_id=users.id ";

        $where_auto = " projecttasks.deleted=0 ";
    ...
    */
    function getProjectTaskParent()
    {
        ...
        {
            $query = "SELECT id FROM projecttasks WHERE project_id = '{$this->project_id}' AND project_task_id = '{$this->parent_task_id}' AND deleted = 0 ORDER BY date_modified DESC";
            $project_task_id = $this->db->getOne($query, true, "Error retrieving parent project task");

            if (!empty($project_task_id))
            {
                $projectTaskParent = BeanFactory::getBean('ProjectTasks', $project_task_id);
            }
        }
    ...
    */
    function getAllSubProjectTasks()
    {
        ...
        if (!empty($this->project_task_id) && !empty($this->project_id))
		{
            ...
            $query = "SELECT id, project_task_id, parent_task_id FROM projecttasks WHERE project_id = '{$this->project_id}' AND deleted = 0 ORDER BY project_task_id";
            ...
            foreach($subProjectTasks as $id=>$values)
            {
                ...
                {
                    $projectTaskBean = BeanFactory::getBean('ProjectTasks', $id);
    ...
    private function getNumberOfTasksInProject($project_id='')
    {
    	if(!empty($project_id))
    	{
	        $query = "SELECT count(project_task_id) AS total FROM projecttasks WHERE project_id = '{$project_id}'";
    ...
    public function updateStatistic()
    {
        ...
        {
            $task = BeanFactory::getBean('ProjectTasks');
